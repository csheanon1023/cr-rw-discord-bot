...
exports.getMembersByLevel = async (message, args, accessLevel) => {
  ...
  if(!Object.keys(clanCodes)?.includes(args[0].toLowerCase()))
    return message.reply(`Please check the clan code! Valid codes are ${clanCodes.keys.join(',')}`);
  ...
  const responseData = [];
  for(index = 0; index < filteredByLevel.length; index++) {
...
const formatPlayerData = (playerData) => {
  ...
  for(i = 0; i < playerCards.length; i++) {
    ...
    if(i < 12) {
      if(groupedByLevel[card.level] == null)
        groupedByLevel[card.level] = [];
      groupedByLevel[card.level].push(card.name);
      continue;
    }
    if(card.level == lowerLevelCount)
      otherCardsWithSameLevel.push(card.name);
    else
      break;
  }
